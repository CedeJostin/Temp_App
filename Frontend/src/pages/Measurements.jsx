...
// ── UI helpers ────────────────────────────────────────────────
const S = { background: '#1e293b', border: '1px solid #334155', borderRadius: 12, padding: 20, marginBottom: 16 }

const StatBox = ({ label, value, color }) => (
  <div style={{ background: '#0f172a', borderRadius: 8, padding: '8px 12px', textAlign: 'center', minWidth: 78 }}>
...
const QualityBadge = ({ ok, label, value, target }) => (
  <div style={{ background: ok ? '#22c55e15' : '#ef444415', border: `1px solid ${ok ? '#22c55e30' : '#ef444430'}`, borderRadius: 8, padding: '6px 10px', fontSize: 11 }}>
    ...
    <div style={{ color: '#94a3b8', marginTop: 2 }}>{value} <span style={{ color: '#64748b' }}>({target})</span></div>
  </div>
)
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'collapse', fontSize: 10, minWidth: 580 }}>
        <thead>
          <tr>
            <th style={{ padding: '3px 8px', color: '#64748b', textAlign: 'left' }}>
              Mes\{isHour ? 'Hora' : 'Semana'}
            </th>
            {ejeVals.filter((_, i) => isHour || i % 2 === 0).map(e => (
              <th key={e} style={{ padding: '2px 1px', color: '#64748b', minWidth: isHour ? 28 : 20, fontWeight: 400 }}>
        ...
        <tbody>
          {MESES.map((m, mi) => (
            <tr key={m}>
              <td style={{ padding: '2px 8px', color: '#94a3b8', fontWeight: 600 }}>{m}</td>
              {ejeVals.filter((_, i) => isHour || i % 2 === 0).map(e => {
                const v = get(mi, e)
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 8, fontSize: 11, color: '#64748b' }}>
        <span>{min}{unit}</span>
        <div style={{ flex: 1, height: 8, borderRadius: 4, background: `linear-gradient(to right, #0f172a, ${color})` }} />
        <span>{max}{unit}</span>
      </div>
    </div>
  )
...
function ResultCard({ variable, resultado }) {
  ...
  return (
    <div style={{ background: '#1e293b', border: `1px solid ${variable.color}30`, borderTop: `3px solid ${variable.color}`, borderRadius: 12, marginBottom: 24 }}>

      {/* Header */}
      <div style={{ padding: '16px 20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        <div>
          <span style={{ fontWeight: 700, fontSize: 15, color: '#f1f5f9' }}>{variable.label}</span>
      ...
      {/* Tabs */}
      <div style={{ padding: '0 20px', borderTop: '1px solid #334155', display: 'flex', gap: 2, flexWrap: 'wrap' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            padding: '9px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer',
            background: 'none', border: 'none',
            color: tab === t.id ? variable.color : '#94a3b8',
        ...
        {tab === 'serie' && (
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={serie} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="period" tick={{ fontSize: 9, fill: '#94a3b8' }} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 9, fill: '#94a3b8' }} unit={variable.unit} />
        ...
        {tab === 'fdp' && (
          <div>
            ...
            {curvas.length > 0 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                {curvas.map((c, i) => (
                  <div key={i} style={{ background: '#0f172a', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
                    <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>{label} {i+1}</div>
                    {tipo_curva === 'gaussiana' && <>
                      <div style={{ color: '#f1f5f9' }}>μ = <strong>{c.mu}{variable.unit}</strong></div>
                      <div style={{ color: '#f1f5f9' }}>σ = <strong>{c.sigma}</strong></div>
                      <div style={{ color: '#f1f5f9' }}>w = <strong>{(c.w*100).toFixed(1)}%</strong></div>
                    </>}
                    {tipo_curva === 'beta' && <>
                      <div style={{ color: '#f1f5f9' }}>Moda ≈ <strong>{c.mode}%</strong></div>
                      <div style={{ color: '#f1f5f9' }}>Var = <strong>{c.variance}</strong></div>
                      <div style={{ color: '#f1f5f9' }}>α={c.alpha} β={c.beta}</div>
                      <div style={{ color: '#f1f5f9' }}>w = <strong>{(c.w*100).toFixed(1)}%</strong></div>
                    </>}
                    {tipo_curva === 'weibull' && <>
                      <div style={{ color: '#f1f5f9' }}>λ = <strong>{c.lambda} m/s</strong></div>
                      <div style={{ color: '#f1f5f9' }}>k = <strong>{c.k}</strong></div>
                      <div style={{ color: '#f1f5f9' }}>vmax = <strong>{c.vmax} m/s</strong></div>
                      <div style={{ color: '#f1f5f9' }}>σ = <strong>{c.sigma}</strong></div>
                      <div style={{ color: '#f1f5f9' }}>w = <strong>{(c.w*100).toFixed(1)}%</strong></div>
                    </>}
                  </div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={fdp} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="x" tick={{ fontSize: 9, fill: '#94a3b8' }} unit={variable.unit}
            ...
            {/* Gráfico de residuos */}
            <p style={{ fontSize: 12, color: '#64748b', marginTop: 16, marginBottom: 6 }}>Residuos (error real vs modelo)</p>
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={fdp} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="x" tick={{ fontSize: 9, fill: '#94a3b8' }} unit={variable.unit} interval={Math.ceil(fdp.length / 10)} />
                <YAxis tick={{ fontSize: 9, fill: '#94a3b8' }} />
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {[['hour','Mes × Hora'], ['week','Mes × Semana']].map(([v, l]) => (
                <button key={v} onClick={() => setHeatType(v)} style={{
                  ...
                  background: heatType === v ? variable.color : '#334155',
                  color:      heatType === v ? '#000' : '#94a3b8',
                }}>{l}</button>
              ))}
            </div>
          ...
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
              <button onClick={() => setMes('annual')} style={{
                ...
                background: mes === 'annual' ? variable.color : '#334155',
                color:      mes === 'annual' ? '#000' : '#94a3b8',
              }}>Anual</button>
              {MESES.map((m, i) => (
                <button key={i} onClick={() => setMes(String(i+1))} style={{
                  padding: '4px 10px', borderRadius: 6, border: 'none', fontSize: 11, cursor: 'pointer',
                  background: mes === String(i+1) ? variable.color : '#334155',
                  color:      mes === String(i+1) ? '#000' : '#94a3b8',
                }}>{m}</button>
              ))}
            </div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              ...
              >
                ...
                <Line type="monotone" dataKey="q25"  name="Q25"      stroke="#94a3b8"        strokeWidth={1} strokeDasharray="2 3" dot={false} />
                <Line type="monotone" dataKey="q75"  name="Q75"      stroke="#94a3b8"        strokeWidth={1} strokeDasharray="2 3" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={annual_profile} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="doy" tick={{ fontSize: 9, fill: '#94a3b8' }} label={{ value: 'Día del año', position: 'insideBottom', offset: -5, fontSize: 10, fill: '#64748b' }} />
                <YAxis tick={{ fontSize: 9, fill: '#94a3b8' }} unit={variable.unit} />
                <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, fontSize: 12 }}
          ...
          <div style={{ overflowX: 'auto' }}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={k} style={{ borderBottom: '1px solid #1e293b' }}>
                    <td style={{ padding: '7px 12px', color: '#94a3b8' }}>{k}</td>
                    <td style={{ padding: '7px 12px', color: '#f1f5f9', fontWeight: 500 }}>{v}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {curvas.length > 0 && (
              <div style={{ marginTop: 16 }}>
                <p style={{ fontSize: 12, color: '#64748b', marginBottom: 8, fontWeight: 600 }}>Parámetros por componente</p>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #334155' }}>
                      {tipo_curva === 'gaussiana' && ['Curva','μ','σ','w'].map(h => <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: '#64748b' }}>{h}</th>)}
                      {tipo_curva === 'beta'      && ['Curva','α','β','Moda','Var','w'].map(h => <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: '#64748b' }}>{h}</th>)}
                      {tipo_curva === 'weibull'   && ['Curva','λ','k','vmax','σ','w'].map(h => <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: '#64748b' }}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {curvas.map((c, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #1e293b' }}>
                        ...
                        {tipo_curva === 'gaussiana' && <>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.mu} {variable.unit}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.sigma}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{(c.w*100).toFixed(1)}%</td>
                        </>}
                        {tipo_curva === 'beta' && <>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.alpha}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.beta}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.mode}%</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.variance}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{(c.w*100).toFixed(1)}%</td>
                        </>}
                        {tipo_curva === 'weibull' && <>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.lambda} m/s</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.k}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.vmax} m/s</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{c.sigma}</td>
                          <td style={{ padding: '6px 10px', color: '#f1f5f9' }}>{(c.w*100).toFixed(1)}%</td>
                        </>}
                      </tr>
            ...
            {stats.huecos.length > 0 && (
              <div style={{ marginTop: 14, background: '#f59e0b15', border: '1px solid #f59e0b30', borderRadius: 8, padding: '10px 14px' }}>
                ...
                {stats.huecos.map((h, i) => (
                  <p key={i} style={{ fontSize: 12, color: '#94a3b8' }}>{h.inicio} → {h.fin} ({h.horas} horas)</p>
                ))}
              </div>
            )}
          </div>
...
function CombinedCard({ combined }) {
  ...
  return (
    <div style={{ background: '#1e293b', border: '1px solid #6366f130', borderTop: '3px solid #6366f1', borderRadius: 12, marginBottom: 24 }}>
      <div style={{ padding: '16px 20px 0' }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: '#f1f5f9' }}>Análisis combinado T × HR</span>
        <span style={{ fontSize: 12, color: '#94a3b8', marginLeft: 10 }}>
          {combined.total_paired?.toLocaleString()} pares · Humectación: {combined.humect_pct}%
        </span>
      </div>

      <div style={{ padding: '0 20px', borderTop: '1px solid #334155', marginTop: 12, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            padding: '9px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer',
            background: 'none', border: 'none',
        ...
        {tab === 'density' && (
          <div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
              {[['90','#22c55e'],['95','#f59e0b'],['99','#ef4444'],['out','#64748b']].map(([c, col]) => (
            ...
            <ResponsiveContainer width="100%" height={360}>
              <ScatterChart margin={{ top: 8, right: 16, left: -10, bottom: 20 }}>
                ...
                <YAxis dataKey="HR" name="HR" unit="%" type="number" domain={[0,100]} tick={{ fontSize: 10, fill: '#94a3b8' }}
                ...
                {['90','95','99','out'].map(c => (
                  <Scatter key={c} name={c === 'out' ? 'Fuera 99%' : `≤${c}%`}
                    ...
                    fill={c === '90' ? '#22c55e' : c === '95' ? '#f59e0b' : c === '99' ? '#ef4444' : '#64748b'}
                    fillOpacity={0.6}
                  />
                ))}
              </ScatterChart>
        ...
        {tab === 'mobility' && combined.mobility && (
          <div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ScatterChart margin={{ top: 8, right: 16, left: 0, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="hora" name="Hora" type="number" domain={[0,23]} tick={{ fontSize: 10, fill: '#94a3b8' }}
                  label={{ value: 'Hora del día', position: 'insideBottom', offset: -8, fontSize: 11, fill: '#94a3b8' }} />
                <YAxis dataKey="T_avg" name="T media" unit="°C" tick={{ fontSize: 10, fill: '#94a3b8' }} />
                <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, fontSize: 12 }}
                  formatter={(v,n) => [typeof v === 'number' ? v.toFixed(2) : v, n]} />
  ...
  return (
    <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 10, padding: '14px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <span style={{ fontWeight: 600, fontSize: 13, color: '#f1f5f9' }}>{variable.label}</span>
      ...
      <div
        ...
        style={{
          border: `2px dashed ${drag ? variable.color : archivo ? '#22c55e' : '#334155'}`,
        ...
        {archivo ? (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 13, color: '#f1f5f9' }}>
              📄 {archivo.name} <span style={{ color: '#64748b', fontSize: 11 }}>({(archivo.size/1024).toFixed(1)} KB)</span>
            </span>
  ...
  return (
    <div style={{ padding: '2rem 2.5rem', maxWidth: 1300 }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ margin: '0 0 4px', fontSize: '1.75rem', fontWeight: 800, color: '#f1f5f9' }}>Mediciones</h1>
      ...
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 12, padding: 20, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
          <div>
            <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>N° de componentes para el ajuste</label>
            <div style={{ display: 'flex', gap: 4 }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setNComponents(n)} style={{
                  padding: '4px 12px', borderRadius: 6, border: 'none', fontSize: 12, cursor: 'pointer',
                  background: nComponents === n ? '#6366f1' : '#334155',
        ...
        {errorGlobal && (
          <div style={{ marginBottom: 12, padding: '8px 12px', background: '#ef444415', border: '1px solid #ef444430', borderRadius: 6, fontSize: 13, color: '#ef4444' }}>
            ⚠️ {errorGlobal}
          </div>
        )}
        ...
        <button
          ...
          style={{
            ...
            background: hayArchivos && !algoProcesando ? '#22c55e' : '#334155',
            color:      hayArchivos && !algoProcesando ? '#000'    : '#64748b',
          }}
        >
          {algoProcesando ? '⏳ Procesando en el backend (puede tardar con datos grandes)…' : 'Analizar archivos'}
        </button>
      </div>
      ...
      {VARIABLES.map(v => {
        ...
        if (proc) return (
          <div key={v.key} style={{ ...S, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>
            ⏳ Calculando {v.label} — ajustando {nComponents} curvas {v.key === 'temperatura' ? 'Gaussianas' : v.key === 'humedad' ? 'Beta' : 'Weibull'}…
          </div>
        )
        if (err) return (
          <div key={v.key} style={{ ...S, color: '#ef4444', fontSize: 13 }}>
            ❌ <strong>{v.label}:</strong> {err}
          </div>
        )
        if (!res) return null
